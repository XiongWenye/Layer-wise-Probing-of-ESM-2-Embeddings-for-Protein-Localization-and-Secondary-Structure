...
\graphicspath{{../results/figures/}{figure/}}
...
\caption{Project workflow. Protein sequences are converted to frozen ESM-2 hidden states, pooled or kept at token level, and passed to shallow probes for localization and secondary-structure prediction. The report focuses on F1/Jaccard for localization and Q3 accuracy for secondary structure, with HPA and CB513 used only after model selection.}
\label{fig:workflow}
\end{figure*}
...
Several limitations constrain the claims. First, the layer/pooling grid is complete only for ESM-2 35M layers 6 and 12; the ESM-2 150M layer-30 result is an attention-only extension, not a full 150M pooling sweep. 
Second, we did not run the one-hot CNN or BiLSTM baselines proposed initially, so the localization baseline comparison is against classical sequence-composition features rather than learned sequence models. 
Finally, due to time and compute constraints, we did not run 650M probes or a full-depth ESM-2 sweep. 
These limitations mean that we should frame the findings as a scoped probing study rather than a comprehensive benchmark.
